...
void insertBuffer(struct Darray *Buffer, struct pkt packet)
{
  ...
  {
    Buffer->maxsize *= 2;
    Buffer->array = realloc(Buffer->array, Buffer->maxsize * sizeof(struct pkt));
  }
  Buffer->array[Buffer->currentsize] = packet;
  Buffer->currentsize++;
}
  ...
  struct pkt packet;
{
  ...
  {
      printf("    Wrong ACK for seq # %d, expected %d", packet.acknum,A.lastACK);
      return;
  }
  //add the new packet
